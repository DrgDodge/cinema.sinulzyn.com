...
<div class="min-h-screen bg-gray-950 text-white flex flex-col">
    ...
    <header class="p-6 border-b border-gray-800 flex justify-between items-center bg-gray-950/50 backdrop-blur-md sticky top-0 z-50">
        <div>
            ...
            <p class="text-xs text-gray-500">{tickets.length} tickets in group</p>
        </div>
        <div class="flex gap-2">
            ...
            <a href="/passes" class="bg-gray-800 hover:bg-gray-700 text-white p-2 rounded-lg transition-colors">
            ...
            <div class="w-full h-full overflow-x-auto snap-x snap-mandatory flex items-center gap-6 px-10 no-scrollbar pb-10">
                {#each tickets as ticket}
                    <div class="snap-center shrink-0 w-full max-w-[320px] sm:max-w-sm">
                        <!-- Digital Pass Cutout View (Reuse from [id]) -->
                ...
                <!-- Spacer for last item scroll alignment -->
        ...
        <div class="fixed inset-0 z-[100] bg-black/90 backdrop-blur-md flex items-end sm:items-center justify-center p-4">
            <div class="bg-gray-900 border border-gray-800 rounded-t-[2.5rem] sm:rounded-[2.5rem] w-full max-w-lg p-8 shadow-2xl animate-in slide-in-from-bottom duration-300">
                <div class="flex justify-between items-center mb-8">
                    ...
                    <button onclick={() => showAddMenu = false} class="text-gray-500 hover:text-white transition-colors">
                ...
                {#if availableTickets.length === 0}
                    <div class="text-center py-10 opacity-50">
                        <p class="text-sm">No available ungrouped tickets found.</p>
                    </div>
                {:else}
                        ...
                        {#each availableTickets as ticket}
                            <form method="POST" action="?/addTicket" use:enhance>
                                ...
                                <button type="submit" class="w-full text-left bg-gray-950 border border-gray-800 p-4 rounded-2xl flex items-center gap-4 hover:border-blue-500/50 transition-all active:scale-[0.98]">
                                    <div class="w-10 h-10 rounded-lg bg-blue-900/30 flex items-center justify-center text-blue-400 font-bold">
                                        {ticket.movie.charAt(0)}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="font-bold text-sm truncate">{ticket.movie}</p>
                                        <p class="text-[10px] text-gray-500 uppercase tracking-wider">{ticket.date} • {ticket.time}</p>
                                    </div>
                                    <div class="ml-auto bg-blue-600 text-white p-1 rounded-full">
                ...
                <button 
                    ...
                    class="w-full mt-8 bg-gray-800 text-gray-400 font-bold py-4 rounded-2xl hover:text-white transition-colors"
                >
                    Done
                </button>
            </div>
